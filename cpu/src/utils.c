...
{	
	...
	int socket_servidor_cpu = socket(servinfo->ai_family,
	...
	err = bind(socket_servidor_cpu, servinfo->ai_addr, servinfo->ai_addrlen);
	if(err != 0)
	{
	...
	err = listen(socket_servidor_cpu, SOMAXCONN);
	if(err != 0)
	{
	...
	return socket_servidor_cpu;
}

int esperar_cliente(int socket_servidor_cpu)
{
	// Aceptamos un nuevo cliente
	int socket_cliente_cpu = accept(socket_servidor_cpu, NULL, NULL);
...
int crear_conexion(char *ip, char* puerto)
{
	...
	err = connect(socket_conexion_destino_memoria, destino_info->ai_addr, destino_info->ai_addrlen);
	if(err != 0)
	{
	...
	return socket_conexion_destino_memoria;
}
